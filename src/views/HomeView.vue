      ...
      <table class="w-full border-collapse">
        ...
        <tbody>
          <tr v-for="(country, index) in paginatedCountries" :key="country.cca3">
            ...
            <td class="p-2 text-left">{{ country.name.nativeName }}</td>
            <td class="p-2 text-left">{{ country.altSpellings.join(', ') }}</td>
            <td class="p-2 text-left">{{ country.idd }}</td>
          </tr>
        </tbody>
      </table>

      <div class="flex justify-end items-center gap-2">
        <button v-for="page in totalPages" :key="page" @click="changePage(page)" class="px-4 py-2  text-white rounded-md"
          :class="page === currentPage ? 'bg-red-400' : 'bg-red-600'">
          {{ page }}
        </button>
      </div>
    </div>
